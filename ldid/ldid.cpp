...
public:
  Data(void *base, size_t size) : base_(base),
                                  size_(size),
                                  swapped_(false)
  {
  }
...
public:
  MachHeader(void *base, size_t size) : Data(base, size)
  {
...
class FatMachHeader : public MachHeader
{
private:
  fat_arch *fat_arch_;

public:
  FatMachHeader(void *base, size_t size, fat_arch *fat_arch) : MachHeader(base, size),
                                                               fat_arch_(fat_arch)
  {
  }
...
public:
  FatHeader(void *base, size_t size) : Data(base, size)
  {
...
public:
  Pointer(const MachHeader *framework = NULL, const Target_ *pointer = NULL) : framework_(framework),
                                                                               pointer_(pointer)
  {
  }
...
struct CodesignAllocation
{
  ...
  CodesignAllocation(FatMachHeader mach_header, size_t offset, size_t size, size_t alloc, size_t align) : mach_header_(mach_header),
                                                                                                          offset_(offset),
                                                                                                          size_(size),
                                                                                                          alloc_(alloc),
                                                                                                          align_(align)
...
int ldid_main(int argc, const char *argv[])
{
  union {
    uint16_t word;
    uint8_t byte[2];
...
int ldid2_main(int argc, const char *argv[])
{
  union {
    uint16_t word;
    uint8_t byte[2];
...
extern "C"
{
  ...
  int ldid2_S(const char *codePath, const char *entitlementPath)
  {
    char str[512];
